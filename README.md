...
- **通用 Agent 节点（双通道）**：拖入即用的内联 Agent，或先定义通用 Agent 实体（注册式复用 / 内联 ad-hoc），支持 prompt / skill / mcp 资源引用
- **子图编排（graph-in-graph）**：节点内嵌子图或引用目录中已有图，支持下钻编辑与面包屑返回；编译期循环引用检测 + 嵌套深度（≤3 层）防护
