...
| 模块 | 职责 |
|------|------|
| `ace-graph-dsl-core` | DSL 模型、编译器、运行时、注册中心、**Aviator / SpEL** 脚本引擎、安全控制 |
| `ace-graph-dsl-script-qlexpress` | **可选**：QLExpress 引擎实现（宿主显式引入） |
| `ace-graph-dsl-script-groovy` | **可选**：Groovy Sandbox 引擎（默认关闭，需显式开启） |
| `ace-graph-dsl-persistence` | 持久化实现：SQLite（默认）、Redis、JDBC、内存 |
| `ace-graph-dsl-spring-boot-starter` | Spring Boot 自动配置 + REST Controller；对 script-* 为 optional 依赖 |
...
`script/` 包提供可插拔 **`ScriptEngine` SPI**，允许在运行时通过 API 创建脚本节点，**无需编写 Java 代码、无需重新部署**：

```
ScriptNodeEditor / PropertyPanel
        │ GET /nodes/engines（ScriptEngineDescriptor）
        ▼
ScriptEngineRegistry ── Aviator / SpEL（core）
                     ── QLExpress（optional 模块）
                     ── Groovy（optional，默认关闭）
        │
        ▼ AbstractTimeoutScriptEngine + ScriptOutputNormalizer
```

- `DynamicNodeDefinition` 持久化脚本节点定义（脚本体、引擎、输入/输出 key、权限标签等）。
- `ScriptNodeFactory` 将定义转为 `ScriptRegisteredGraphNode` 并注册到 `GraphNodeRegistry`。
- 条件边可配置 `condition` + `conditionEngine`，经 `ScriptEdgeActionFactory` 编译为路由 `EdgeAction`。
- 样例与选型见 [`SCRIPT_NODE_EXAMPLES.md`](./SCRIPT_NODE_EXAMPLES.md)；方案见 [`MULTI_SCRIPT_ENGINE_PLAN.md`](./MULTI_SCRIPT_ENGINE_PLAN.md)。
- 设计背景见 [`NODE_FLEXIBILITY_EXPLORATION.md`](./NODE_FLEXIBILITY_EXPLORATION.md)。
