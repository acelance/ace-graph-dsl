export default {
  common: {
    cancel: '取消',
    confirm: '确认',
    refresh: '刷新',
    create: '创建',
    close: '关闭',
    loading: '加载中…',
    noData: '暂无数据'
  },
  toolbar: {
    title: 'Graph DSL 设计器',
    save: '保存草稿',
    validate: '校验',
    preview: 'PlantUML 预览',
    publish: '发布',
    versionHistory: '版本历史',
    undo: '撤销',
    redo: '重做',
    dryRun: '试运行',
    dryRunTitle: '试运行',
    debugStream: '调试流式',
    import: '导入',
    export: '导出',
    conditionalEdge: '条件边',
    conditionalEdgeHint: '开启后，下一条连线将直接创建为条件边',
    topology: '拓扑校验',
    zoomIn: '放大',
    zoomOut: '缩小',
    fitView: '适应画布',
    resetZoom: '重置缩放',
    autoLayout: '自动布局',
    minimap: '缩略图',
    minimapHide: '隐藏缩略图',
    minimapShow: '显示缩略图',
    group: '子流程',
    groupCreate: '成组',
    groupCreateHint: '请先多选至少 2 个业务节点再成组（Ctrl+点击，或先开「框选」拖选）',
    groupHint: '用法：Ctrl+点击多选节点，或点「框选」后拖选，再点本按钮成组。仅画布可视化，不写入 DSL。',
    boxSelect: '框选',
    boxSelectHint: '框选已开启：在画布空白处拖拽框住节点，松开后可点子流程成组',
    boxSelectUnavailable: '框选插件不可用',
    groupCollapse: '折叠',
    groupExpand: '展开',
    groupUngroup: '解组',
    extractSubgraph: '提取为子图',
    extractSubgraphHint: '选中节点（Ctrl+点击多选，或开「框选」拖选）后，将其折叠为一个子图节点',
    groupEmpty: '暂无子流程分组',
    groupCount: '{n} 个节点',
    importSuccess: '已导入 DSL（{graphId} v{version}），记得保存',
    importFailed: '导入失败: {msg}',
    importInvalid: '文件不是有效的 Graph DSL JSON',
    exportSuccess: '已导出 DSL JSON',
    saveSuccess: '草稿已保存',
    saveFailed: '保存失败: {msg}',
    validateSuccess: '校验通过',
    validateFailed: '校验失败，{count} 个错误',
    previewSuccess: 'PlantUML 已生成，查看属性面板',
    previewFailed: '预览失败: {msg}',
    publishConfirm: '确认发布当前版本？发布后将切换运行时 Graph。',
    publishTitle: '发布确认',
    publishSuccess: '发布成功，版本 {version}',
    publishFailed: '发布失败: {msg}',
    saveUnchanged: '相对基线版本无内容变更，未生成新版本',
    saveCollapsed: '已保存（含内联子图），已回到根图',
    readOnly: '内置图 · 只读',
    statusRunning: '运行中 v{version}',
    statusDraft: '目标 v{version}',
    statusBaseline: '基线 v{version}',
    statusUnpublished: '未发布',
    versionBumpTitle: '请升级版本号',
    versionBumpMessage: '检测到 DSL 内容已变更。版本号 {current} 已存在或未大于最大版本 {max}，须使用新版本号保存。是否自动升级为 {suggested}？',
    versionBumpConfirm: '升级为 {suggested}'
  },
  canvas: {
    extractNeedNodes: '请先选中至少 1 个业务节点（Ctrl+点击多选，或开「框选」拖选）',
    extractSuccess: '已将 {n} 个节点提取为子图',
    delete: '删除',
    deleteSelection: '删除选中',
    deleteSelectionHint: '删除选中的节点或连线（Delete / Backspace）',
    pasteHint: 'Ctrl+C 复制选中节点 / Ctrl+V 粘贴',
    isolatedNodes: '存在孤立节点',
    autoLayoutFailed: '自动布局失败',
    copied: '已复制 {n} 个节点',
    legendStreaming: '流式 / 异步节点',
    loadingSubgraph: '正在加载子图…'
  },
  manager: {
    title: 'Graph DSL 管理中心',
    createGraph: '新建 Graph DSL',
    emptyCatalog: '暂无 DSL，请新建',
    emptyCatalogFiltered: '当前上下文无可用图',
    catalogFilterPlaceholder: '按 graphId 精确过滤（失焦生效）',
    otherBizParamsTooLong: 'otherBizParams 超过 {max} 字节上限，已拒绝传递',
    selectOrCreate: '请选择或新建 Graph DSL',
    createDialogTitle: '新建 Graph DSL',
    graphId: 'graphId',
    graphIdPlaceholder: '如 order-flow / cs-reply-m2',
    displayName: '显示名',
    displayNamePlaceholder: '业务显示名称'
  },
  version: {
    title: '版本历史',
    tabPublished: '已发布',
    tabDrafts: '草稿历史',
    enabled: '当前运行',
    editing: '编辑中',
    publishedHint: '业务运行时正在使用的版本',
    noPublished: '尚未发布',
    compare: '与当前对比',
    rollback: '回滚到此版本',
    rollbackConfirm: '确认回滚到版本 {version}？运行时将切换为该版本。',
    rollbackTitle: '回滚确认',
    rollbackSuccess: '已回滚到版本 {version}',
    rollbackFailed: '回滚失败: {msg}',
    loadFailed: '加载版本失败: {msg}',
    loadBaseline: '设为编辑基线',
    loadBaselineSuccess: '已加载 v{version} 为编辑基线',
    tabStructure: '结构对比',
    tabJson: 'JSON 对比',
    added: '新增',
    removed: '删除',
    modified: '修改',
    nodes: '节点',
    edges: '边',
    compile: '编译配置',
    meta: '元信息',
    noDiff: '与当前无差异',
    selectVersion: '请选择左侧版本查看详情',
    baseLabel: '当前（编辑器）',
    targetLabel: '选中版本'
  },
  nodePanel: {
    title: '节点面板',
    collapse: '收起',
    expand: '展开',
    canvasHint: '画布已内置 START / END 节点，请从 START 连出第一条边开始绘制。',
    search: '搜索节点',
    all: '全部',
    builtin: '内置',
    script: '脚本',
    createScript: '+ 新建脚本节点',
    edit: '编辑',
    delete: '删除',
    deleteConfirm: '确认删除脚本节点 "{name}"？',
    referenceWarning: '以下图引用了此节点，删除后将无法编译:',
    empty: '无节点',
    structural: '结构节点',
    subgraph: '子图',
    agent: 'Agent 节点',
    genericAgent: '通用 Agent 节点',
    agentTab: '通用 Agent',
    createAgent: '+ 新建通用 Agent',
    deleteAgentConfirm: '确认删除通用 Agent 节点 "{name}"？'
  },
  edgeValidation: {
    title: '连线参数校验',
    issue: '{from} → {to}：目标节点 {target} 缺少入参 [{keys}]'
  },
  topology: {
    title: '拓扑校验',
    ok: '拓扑结构正常（无环、END 可达、无孤立节点）',
    cycle: '环',
    noEnd: 'END 不可达',
    unreachable: '不可达节点',
    isolated: '孤立节点',
    error: '错误',
    warning: '警告'
  },
  search: {
    placeholder: '搜索节点并定位…',
    noResult: '无匹配节点',
    hint: '输入节点名称或 ID，回车/点击定位到画布'
  },
  group: {
    title: '子流程分组',
    collapsed: '已折叠',
    expanded: '已展开'
  },
  edgeEditor: {
    title: '边配置',
    type: '类型',
    from: '源节点',
    to: '目标节点',
    conditional: '条件边',
    normal: '普通边',
    routingMode: '路由方式',
    dispatcherMode: 'Java Dispatcher',
    scriptMode: '脚本路由',
    engine: '脚本引擎 (conditionEngine)',
    condition: '路由表达式 (condition)',
    conditionHint: '返回 mapping 的 key（字符串）；可用变量 {state}（图状态）',
    mapping: '路由映射 (mapping)',
    mappingKey: '条件值 / key',
    mappingTarget: '目标节点',
    addMapping: '+ 新增映射',
    apply: '应用',
    applyOk: '边配置已应用',
    unknownEngine: '脚本引擎不可用: {engine}（不在当前可用引擎列表中，请重新选择）',
    convert: '转为条件边',
    noConditional: '普通边不支持条件路由。点击「转为条件边」可启用条件路由配置。',
    parallel: '并行',
    parallelHint: '标记为并行分支（同源多目标 = 并行块）；运行时由 StateGraph 默认并发执行',
    aggregation: '聚合策略',
    aggregationNone: '无（默认）',
    aggregationAllOf: 'ALL_OF（全部完成）',
    aggregationAnyOf: 'ANY_OF（任一完成）'
  },
  propertyPanel: {
    title: '属性面板',
    tabNode: '节点配置',
    tabMeta: '图元信息',
    tabCompile: '编译配置',
    tabKeys: 'KeyStrategy',
    tabValidation: '校验结果',
    tabPlantUml: 'PlantUML',
    nodeId: '节点 ID',
    displayName: '显示名',
    origin: '来源',
    inputKeys: 'Input Keys',
    outputKeys: 'Output Keys',
    noConfig: '该节点无可配置属性',
    clickNode: '点击画布节点查看配置',
    graphId: 'Graph ID',
    version: '版本',
    description: '描述',
    saver: 'Saver',
    interruptBefore: '中断点',
    interruptPlaceholder: '选择 HITL 节点',
    stateKey: 'State Key',
    strategy: '策略',
    addKey: '新增 Key',
    validationOk: '校验通过',
    previewHint: '点击工具栏「预览」生成 PlantUML',
    agentNote: 'Agent 节点为代码岛式节点：运行时由后端注册的同类 Agent 动作处理，前端仅记录节点与配置。',
    subgraphMode: '子图模式',
    inline: '内联',
    reference: '引用',
    subgraphRef: '引用图 ID',
    subgraphRefPlaceholder: '从下拉选择一张已存在的图',
    subgraphRefHint: '选好后节点会以「引用」方式指向该图，双击节点或点「进入子图」可查看引用目标',
    subgraphRefEmpty: '暂无可引用的图',
    subgraphRefEmptyHint: '需先在左侧目录中创建并保存至少一张图，才能作为子图引用',
    subgraphVersion: '版本锁定',
    subgraphVersionLatest: '最新（默认）',
    subgraphVersionLocked: '已锁定版本 {version}',
    subgraphVersionHint: '默认跟随最新版本，可下拉锁定特定版本以避免引用图更新导致行为变化',
    enterSubgraph: '进入子图',
    enterSubgraphHint: '双击画布上的子图节点也可进入',
    previewSubgraph: '预览子图',
    previewSubgraphTitle: '子图拓扑预览',
    previewLoading: '正在生成预览…',
    previewEmpty: '该子图暂无节点',
    previewCompiledNote: '编译视图（服务端生成）',
    previewFallbackNote: '服务端生成失败，已回退为结构视图',
    previewLoadFailed: '预览生成失败：',
    streaming: '流式输出',
    streamingHint: '标记为流式 / 异步节点：输出以流式（逐 token）或异步方式产生，画布节点右上角将以脉冲徽标区分',
    genericAgentNote: '通用 Agent 节点为元数据驱动：填入模型 / promptKeys / skillKeys / mcpKeys / localToolKeys 等元数据后，后端模板节点即可直接执行模型调用，无需内嵌代码。',
    genericAgentRegisteredNote: '该节点为已注册的通用 Agent（引用式）：元数据在「节点面板 → 通用 Agent」中集中定义与管理，此处仅展示节点引用。',
    genericAgentRegisteredTitle: '注册式（引用型）通用 Agent',
    genericAgentEditInPanel: '前往节点面板编辑',
    genericAgentEditInPanelHint: '元数据在「节点面板 → 通用 Agent」中维护',
    agentSpec: {
      modelBaseUrl: '模型 Base URL',
      modelApiKey: 'API Key',
      modelApiKeyMasked: 'API Key（已脱敏）',
      modelApiKeyMaskedHint: '已落库的 API Key 仅留后 4 位。如需更新请直接输入新 Key，留空则保持原值。',
      modelId: '模型 ID',
      modelIdOptionalHint: '与下方「Model Config Key」二选一：内联填此项，或启用资源 Key（Key 含 modelId）。',
      modelIdFromConfigHint: '已配置 Model Config Key，模型 ID 由注册中心解析，无需在此填写。',
      prompt: 'Prompt（内联）',
      promptKey: 'Prompt Key',
      promptHint: 'promptKeys 按序加载后，内联 prompt 作为追加段拼在其后（可同时使用）。',
      skill: 'Skill',
      skillKey: 'Skill Key',
      skillHint: 'skill / skill-key 用于从 SkillRegistry 加载技能资源，二者可同时为空。',
      mcp: 'MCP',
      mcpKey: 'MCP Key',
      inputKeys: 'Input Keys',
      inputKeysHint: '逗号分隔，从图状态读取这些 key 作为模型调用的 variables。',
      outputKey: 'Output Key',
      outputKeyHint: '模型回复写入图状态的 key，默认 agent_result。',
      mediaInputKey: '多模态入参 Key',
      mediaInputKeyHint: '从哪个 state key 读 MediaRef 列表；留空=纯文本。建议同步写入 inputKeys 便于连线校验。',
      streamResponseKind: '流式类型',
      streamResponseKindHint: '从已实现类型目录选择；空值编译期回落目录首位（默认 BIZ）。',
      streamResponseKindLoadFail: '流式类型目录加载失败，已使用默认项',
      memoryMode: '对话记忆',
      memoryModeHint: 'NONE=不参与；READ_ONLY=只灌历史不写 ASSISTANT；READ_WRITE=本节点 LLM 完成后立即读写记忆（非图尾统一落盘）。',
      memoryModeNone: 'NONE（不参与）',
      memoryModeReadOnly: 'READ_ONLY（只读历史）',
      memoryModeReadWrite: 'READ_WRITE（读写，节点完成即落盘）',
      deepThinking: '深度思考',
      applyDeepThinking: '应用请求深度思考',
      applyDeepThinkingHint: '开启后，本节点才会应用前端传入的深度思考开关（经业务映射写入框架键）；意图/输出等节点建议关闭。',
      bizParams: '业务附加参数',
      enableBizParams: '启用业务附加参数',
      enableBizParamsHint: '开启后须选择解释器；框架只存原文，由业务解释器解析。',
      bizParamInterpreterId: '解释器',
      bizParamInterpreterIdHint: '业务侧实现的解析方式；未实现时可选「纯文本（默认）」。',
      bizParamRaw: '附加参数内容',
      bizParamRawPlaceholder: '{"thinking":true,"nodeDisplay":"业务处理"}',
      bizParamRawHint: 'JSON 或纯文本均可；具体语义由所选解释器决定。',
      // P0.5 资源两级勾选
      resources: '资源勾选',
      enablePrompt: '启用 Prompt 资源',
      promptKeys: 'Prompt Keys',
      promptKeysHint: '逗号分隔，多 key 按序合并后再追加内联 prompt。',
      enableModel: '启用 Model 配置 Key',
      modelConfigKey: 'Model Config Key',
      modelConfigKeyHint: '勾选后走注册中心整路配置；勿与内联字段拼盘。',
      enableMcp: '启用 MCP',
      mcpKeys: 'MCP Keys',
      mcpKeysHint: '目录未加载时的退路：逗号分隔的 MCP server key。不要把目录外的字回车当成已选。',
      mcpTree: 'MCP 资源（三级树）',
      mcpTreeHint: '勾选 server；展开可勾具体工具。不勾工具 = 该 server 全选。server 显示为资源编码(显示名称)，工具只显示编码。',
      catalogEmpty: '目录为空，本次不能当作选项。',
      catalogLoadFailed: '目录加载失败，本次不能当作选项。',
      mcpAdvancedShow: '高级：文本白名单',
      mcpAdvancedHide: '收起文本白名单',
      mcpToolWhitelist: 'MCP 工具白名单',
      mcpToolWhitelistHint: '格式 serverKey:tool1|tool2；多 server 用分号。留空表示该 server 全选。',
      enableSkill: '启用 Skill',
      skillKeys: 'Skill Keys',
      skillTree: 'Skill 资源（勾选）',
      skillTreeHint: '勾选要启用的 skill；显示为资源编码(显示名称)。写入 skillKeys 的是资源编码。勾选≠预加载正文。',
      skillKeysHint: '目录为空时的退路：逗号分隔的 skill key。有目录时请用上方勾选列表。',
      enableLocalTools: '启用本地工具',
      localToolKeys: 'Local Tool Keys',
      localToolKeysHint: '逗号分隔的本地工具 key。'
    }
  },
  scriptEditor: {
    title: '新建脚本节点',
    editTitle: '编辑脚本节点',
    nodeId: 'Node ID',
    nodeIdPlaceholder: 'script:my_node',
    displayName: '显示名',
    displayNamePlaceholder: '入参标准化',
    category: '类别',
    description: '描述',
    inputKeys: 'Input Keys',
    inputKeysPlaceholder: '逗号分隔，如 query,score',
    outputKeys: 'Output Keys',
    outputKeysPlaceholder: '逗号分隔，如 normalized_query',
    permissionTags: '权限标签',
    permissionTagsPlaceholder: '逗号分隔，如 public,cs',
    engine: '脚本引擎',
    scriptBody: '脚本内容',
    scriptHint: '可用变量：{state}（输入）、{config}（节点配置）；返回 Map 或使用 seq.map(key, value)',
    mockState: 'Mock State',
    testOutput: '试跑结果',
    validateOk: '脚本语法校验通过',
    testOk: '试跑成功',
    mockError: 'mockState JSON 格式错误',
    nodeIdError: 'nodeId 必须以 script: 开头',
    createOk: '脚本节点已创建',
    updateOk: '脚本节点已更新',
    validate: '校验语法',
    testRun: '试跑',
    create: '创建',
    update: '更新'
  },
  agentEditor: {
    title: '新建通用 Agent 节点',
    editTitle: '编辑通用 Agent 节点',
    nodeId: 'Node ID',
    nodeIdPlaceholder: 'agent:my_agent',
    nodeIdError: 'nodeId 必须以 agent: 开头',
    displayName: '显示名',
    displayNamePlaceholder: '如 智能客服 Agent',
    description: '描述',
    model: '模型配置',
    prompt: 'Prompt 配置',
    skill: 'Skill 配置',
    mcp: 'MCP 配置',
    advanced: '高级',
    browseAgentCodePlaceholder: 'agentCode（浏览 Catalog，不落库）',
    refreshCatalog: '刷新',
    permissionTags: '权限标签',
    permissionTagsPlaceholder: '逗号分隔，如 public,cs',
    mockState: 'Mock State',
    testOutput: '试跑结果',
    validate: '校验',
    validateOk: '元数据校验通过',
    testRun: '试跑',
    testOk: '试跑成功',
    mockError: 'mockState JSON 格式错误',
    createOk: '通用 Agent 节点已创建',
    updateOk: '通用 Agent 节点已更新'
  },
  engine: {
    aviator: { hint: "单行表达式；返回 seq.map('key', value) 或标量" },
    spel: { hint: "使用 #state['key'] / #config['key']；Map 用 {'key': value}" },
    qlexpress: { hint: "支持多行 if/else；return map('key', value)" },
    groovy: { hint: "支持集合操作；return [key: value]；需管理员开启" }
  },
  dryRun: {
    title: '试运行',
    hint: '将当前草稿临时编译并运行一次，收集各节点输出轨迹，不影响已发布图。',
    mockState: '初始 State (JSON)',
    run: '运行',
    running: '运行中…',
    trace: '执行轨迹',
    finalState: '最终状态',
    node: '节点',
    empty: '运行后展示各节点输出轨迹',
    error: '试运行失败',
    mockError: 'mockState JSON 格式错误'
  },
  execution: {
    title: '图执行调试（SSE + HITL）',
    hint: '设计器调试走 /execution/{graphId}/debug/stream，输出框架标准 debug 格式；生产业务请自接 /stream + 业务 Formatter。',
    debugFormatHint: '调试所见 ≠ 生产所下发。本面板使用框架 DebugStreamingChunkFormatter（含 responseKind / outputType 等诊断字段）。',
    debugPermissionDenied: '缺少菜单权限 graph:validate，无法使用调试流式执行',
    agentCode: 'Agent Code',
    agentCodePlaceholder: '如 cs-assistant（写入 ace.graph.dsl.agentCode）',
    forceSkills: 'Force Skills',
    forceSkillsPlaceholder: 'skill.refund, skill.invoice',
    forceSkillsHint: '逗号分隔；写入 ace.graph.dsl.forceSkills，须落在节点 skillKeys 白名单内才会预激活。',
    inputs: '输入 State (JSON)',
    inputsError: '输入 JSON 格式错误',
    run: '执行',
    runDebug: '调试执行',
    stop: '停止',
    trace: '执行轨迹',
    error: '执行异常',
    empty: '执行后展示各节点输出事件',
    hitlPaused: 'HITL 暂停 — 等待人工输入',
    subgraphPaused: '子图 {parent} 的节点 {node} 已暂停',
    topLevelPaused: '顶层节点 {node} 已暂停',
    resumeUpdates: '恢复更新 (JSON)',
    resume: '恢复执行',
    updatesError: '更新 JSON 格式错误'
  }
}
