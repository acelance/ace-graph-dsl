export default {
  ...
  manager: {
    ...
    emptyCatalogFiltered: 'No graphs available in current context',
    catalogFilterPlaceholder: 'Exact graphId filter (apply on blur)',
    otherBizParamsTooLong: 'otherBizParams exceeds {max} bytes and was rejected',
  ...
  propertyPanel: {
    ...
    genericAgentNote: 'Generic Agent node is metadata-driven: fill in model / promptKeys / skillKeys / mcpKeys / localToolKeys and the backend template node will execute the model call directly — no embedded code required.',
    ...
    agentSpec: {
      ...
      modelId: 'Model ID',
      modelIdOptionalHint: 'Either fill this inline, or enable Model Config Key below (the key already includes modelId).',
      modelIdFromConfigHint: 'Model Config Key is set; model ID comes from the registry and is not required here.',
      prompt: 'Prompt (inline)',
      promptKey: 'Prompt Key',
      promptHint: 'promptKeys are loaded in order; the inline prompt is appended after them (both can be used together).',
      skill: 'Skill',
      skillKey: 'Skill Key',
      ...
      mediaInputKey: 'Media Input Key',
      mediaInputKeyHint: 'State key holding MediaRef list; leave empty for text-only. Prefer also listing it in inputKeys for edge checks.',
      streamResponseKind: 'Stream Kind',
      streamResponseKindHint: 'Pick from implemented kinds; empty falls back to catalog first (default BIZ) at compile time.',
      streamResponseKindLoadFail: 'Failed to load stream kinds; using defaults',
      memoryMode: 'Chat Memory',
      memoryModeHint: 'NONE=skip; READ_ONLY=inject history without writing ASSISTANT; READ_WRITE=read/write when this node LLM finishes (not graph-end persist).',
      memoryModeNone: 'NONE (skip)',
      memoryModeReadOnly: 'READ_ONLY (history only)',
      memoryModeReadWrite: 'READ_WRITE (persist when node finishes)',
      deepThinking: 'Deep Thinking',
      applyDeepThinking: 'Apply request deep thinking',
      applyDeepThinkingHint: 'When on, this node applies the request-level deep-thinking flag (mapped by business into the framework key); keep off for intent/output nodes.',
      bizParams: 'Business params',
      enableBizParams: 'Enable business params',
      enableBizParamsHint: 'When on, pick an interpreter; the framework only stores raw text.',
      bizParamInterpreterId: 'Interpreter',
      bizParamInterpreterIdHint: 'Business parser implementation; use the default string interpreter when none is provided.',
      bizParamRaw: 'Param content',
      bizParamRawPlaceholder: '{"thinking":true,"nodeDisplay":"Business"}',
      bizParamRawHint: 'JSON or plain text; semantics depend on the selected interpreter.',
      resources: 'Resource Binding',
      enablePrompt: 'Enable Prompt resources',
      promptKeys: 'Prompt Keys',
      promptKeysHint: 'Comma-separated; keys are merged in order, then inline prompt is appended.',
      enableModel: 'Enable Model config key',
      modelConfigKey: 'Model Config Key',
      modelConfigKeyHint: 'When enabled, resolve the full endpoint from registry; do not mix with inline fields.',
      ...
      mcpAdvancedHide: 'Hide text whitelist',
      mcpToolWhitelist: 'MCP tool whitelist',
  ...
  agentEditor: {
    ...
    mcp: 'MCP',
    advanced: 'Advanced',
    browseAgentCodePlaceholder: 'agentCode (catalog browse, not persisted)',
  ...
  execution: {
    title: 'Debug Execution (SSE + HITL)',
    hint: 'Designer debug uses /execution/{graphId}/debug/stream (framework debug format). Production apps should call /stream with their own Formatter.',
    debugFormatHint: 'Debug view ≠ production payload. This panel uses DebugStreamingChunkFormatter (responseKind / outputType diagnostics).',
    debugPermissionDenied: 'Missing menu permission graph:validate; debug stream is disabled',
    agentCode: 'Agent Code',
    agentCodePlaceholder: 'e.g. cs-assistant (written to ace.graph.dsl.agentCode)',
    forceSkills: 'Force Skills',
    forceSkillsPlaceholder: 'skill.refund, skill.invoice',
    forceSkillsHint: 'Comma-separated keys written to ace.graph.dsl.forceSkills; must be in the node skillKeys whitelist to pre-activate.',
